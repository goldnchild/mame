...
uint8_t a2bus_silentype_device::read_c0nx(uint8_t offset)
{
	...
	{
		retval |= m_silentype_printer->read_margin_switch() << SILENTYPE_STATUS;  // margin switch on d7
	}
	
	if (BIT(offset,1))  // c091  (decodes bit a01)
	{
		retval |= m_silentype_printer->read_data() << SERIAL_DATA_Q15; // if datawriteenable is off, read serial data from the printer on d7, otherwise reads serial data out (unimplemented)
...
void a2bus_silentype_device::write_c0nx(uint8_t offset, uint8_t data)
{
	...
	if (BIT(offset,0))  // decodes a0
	{
		...
		m_silentype_printer->write_data(BIT(data, SILENTYPE_DATAWRITEENABLE) & BIT(data,SILENTYPE_DATA));

		u8 shifta = BIT(data, SILENTYPE_SHIFTCLOCKA);  // A is 4
		u8 shiftb = BIT(data, SILENTYPE_SHIFTCLOCKB);  // B is 1
		// shiftb is transitory
		// shifta is latched
		if (shifta)
		{
			m_silentype_printer->write_shiftclock(0);  // inverted
		}
		else
		{
			if (shiftb)
			{
				m_silentype_printer->write_shiftclock(0);
				m_silentype_printer->write_shiftclock(1);
			}	
			else m_silentype_printer->write_shiftclock(1);
		}
		//m_silentype_printer->write_shiftclock(! (BIT(data, SILENTYPE_SHIFTCLOCKA) | BIT(data,SILENTYPE_SHIFTCLOCKB)) );
		m_silentype_printer->write_storeclock( !(BIT(data, SILENTYPE_STORECLOCK)) );  // bit 2
		//dropping storeclock will store data
