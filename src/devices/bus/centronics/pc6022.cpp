...
INPUT_PORTS_START( pc6022 )
	...
	PORT_START("PB")
	PORT_BIT( 0x01, IP_ACTIVE_LOW, IPT_KEYPAD ) PORT_NAME("Pen Right") PORT_CODE(KEYCODE_RIGHT)
	PORT_BIT( 0x02, IP_ACTIVE_LOW, IPT_KEYPAD ) PORT_NAME("Pen Left") PORT_CODE(KEYCODE_LEFT)
	PORT_BIT( 0x04, IP_ACTIVE_LOW, IPT_KEYPAD ) PORT_NAME("Pen Down / Paper Feed") PORT_CODE(KEYCODE_DOWN)
	PORT_BIT( 0x08, IP_ACTIVE_LOW, IPT_KEYPAD ) PORT_NAME("Pen Up") PORT_CODE(KEYCODE_UP)
	...
	PORT_BIT( 0x40, IP_ACTIVE_LOW, IPT_KEYPAD ) PORT_NAME("Color Change") PORT_CODE(KEYCODE_C)
...
void pc6022_device::device_add_mconfig(machine_config &config)
{
	...
	m_cpu->pa_in_cb().set(FUNC(pc6022_device::pa_r));
	m_cpu->pa_out_cb().set(FUNC(pc6022_device::pa_w));
...
DEFINE_DEVICE_TYPE_PRIVATE(PC6022, device_centronics_peripheral_interface, pc6022_device, "pc6022", "NEC PC-6022 Color Plotter Printer")
