...
void bit90_state::bit90_io_map(address_map &map)
{
	...
	map(0xe2, 0xe2).mirror(0x1d).r(FUNC(bit90_state::paddle_2_r));  // also, bit7 is tape in
...
static INPUT_PORTS_START( bit90 )
	...
	PORT_BIT( 0x20, IP_ACTIVE_LOW, IPT_KEYBOARD) PORT_NAME("1") PORT_CODE(KEYCODE_1) PORT_CHAR('1') PORT_CHAR('!')
	...
	PORT_BIT( 0x08, IP_ACTIVE_LOW, IPT_KEYBOARD) PORT_NAME("BS") PORT_CODE(KEYCODE_BACKSPACE) PORT_CHAR(8)
	...
	PORT_BIT( 0x40, IP_ACTIVE_LOW, IPT_KEYBOARD) PORT_NAME("R") PORT_CODE(KEYCODE_R) PORT_CHAR('R')
	...
	PORT_BIT( 0x08, IP_ACTIVE_LOW, IPT_KEYBOARD) PORT_NAME("@") PORT_CODE(KEYCODE_END) PORT_CHAR('@')  PORT_CHAR('_')
	...
	PORT_BIT( 0x80, IP_ACTIVE_LOW, IPT_KEYBOARD) PORT_NAME("K") PORT_CODE(KEYCODE_K) PORT_CHAR('K')
	...
	PORT_BIT( 0x80, IP_ACTIVE_LOW, IPT_KEYBOARD) PORT_NAME("L") PORT_CODE(KEYCODE_L) PORT_CHAR('L')
...
void bit90_state::bit90(machine_config &config)
{
	/* basic machine hardware */
	Z80(config, m_maincpu, XTAL(7'159'090)/2); // 3.579545 MHz
//  m_maincpu->z80_set_m1_cycles(4+1); // 1 WAIT CLK per M1   //does this fix approx 12% slow cassette
//  commenting this z80_set_m1_cycles line makes the cassette output the exact timing of the demo cassette
...
//  m_cass->set_formats(bit90_cassette_formats);  // haven't made a format yet
